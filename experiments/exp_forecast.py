...
def compute_model_stats(model, args, num_iterations=50):
    ...
    training_memory_1 = training_memory/1024
    print(f"Training Memory Usage: {training_memory_1:.3f} GB")
    print(f"Inference Time / iter: {avg_inference_time * 1000:.3f} ms")
    print(f"Inference Memory Usage: {inference_memory:.3f} MB")
    inference_memory_GB = inference_memory/1024
    print(f"Inference Memory Usage: {inference_memory_GB:.3f} GB")
        ...
        for epoch in range(start_epoch, self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if epoch == -1:
                    ...
                    print(f"Total FLOPs  : {flops * 2 / 1e9:.10f} G  (one full forward)")
                    print(f"Total FLOPs  : {flops * 2 / 1e6:.10f} M  (one full forward)")
            ...
            peak_memory = torch.cuda.max_memory_allocated() / 1024 ** 2  # ✅ 打印显存峰值
            print(f"Epoch {epoch + 1} GPU Peak Memory: {peak_memory:.2f} MB")
    ...
    def test(self, setting=None, test=0, test_batch_size=None):
        ...
        if test < 1:
            ...
            full_folder, new_setting = find_most_recently_modified_subfolder(self.args.checkpoints,
                                                                             file_name='checkpoint.pth',
                                                                             contain_str=self.args.model_id_ori)
            # full_folder="F:\code\Time Series\FDAM_TS-main\checkpoints\\forecasting_ETTh1_fdam_96_96_e1_s0_p1_o1_attn_ablation_250919_133008_FDAM_ETTh1_sl-96_pl-96_var-7_dm-1024_stages-4_2P2i_1P1i_dec-2_des-'Exp'_0"
            # new_setting=setting
